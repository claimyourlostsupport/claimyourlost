/**
 * Proxies GET /share/social/:postId to the Render API Open Graph page.
 * Uses middleware so we do not rely on dynamic filenames like [id].js (can fail to deploy on some setups).
 *
 * Env (Cloudflare Pages → Variables): SOCIAL_SHARE_API_ORIGIN, or VITE_API_URL (https only).
 * Fallback: set HARDCODED_API_ORIGIN below.
 */
...
export async function onRequest(context) {
  ...
  if (!apiOrigin) {
    return new Response(
      [
        'Share preview: set SOCIAL_SHARE_API_ORIGIN to your Render API URL in Cloudflare Pages → Variables,',
